...
if prompt := st.chat_input(
    ...
    # Get agent response
    # 1. Import asyncio at the top of your file
import asyncio

# ... (rest of your code)

# 2. Update the response call (Line 85 in image_bd4c56.png)
with st.chat_message("assistant"):
    with st.spinner("Alex is thinking... 🤔"):
        try:
            # Wrap the call in asyncio.run() to execute the coroutine
...
with st.sidebar:
    ...
    if st.button("📋 All Orders",
        ...
        st.session_state.messages.append({
            "role": "user",
            "content": "Show me all sales orders"
        })
        st.rerun()
    ...
    if st.button("✅ Open Orders",
        ...
        st.session_state.messages.append({
            "role": "user",
            "content": "Show me all open orders"
        })
        st.rerun()
    ...
    if st.button("📊 Sales Summary",
        ...
        st.session_state.messages.append({
            "role": "user",
            "content": "Give me a complete sales summary"
        })
        st.rerun()
    ...
    if st.button("Validate Customer",
        ...
        if customer_code:
            st.session_state.messages.append({
                "role": "user",
                "content": f"Please validate customer {customer_code}"
            })
            st.rerun()
    ...
    if st.button("Check Stock",
        ...
        if item_name:
            st.session_state.messages.append({
                "role": "user",
                "content": f"What is the current stock for {item_name}?"
            })
            st.rerun()
    ...
    if st.button("💰 Top Customers",
        ...
        st.session_state.messages.append({
            "role": "user",
            "content": "Which customers have the highest order values?"
        })
        st.rerun()
    ...
    if st.button("📦 Popular Items",
        ...
        st.session_state.messages.append({
            "role": "user",
            "content": "Which items are ordered the most?"
        })
        st.rerun()

    if st.button("📅 Monthly Orders",
                  use_container_width=True):
        st.session_state.messages.append({
            "role": "user",
            "content": "Show me orders grouped by month"
        })
        st.rerun()
    ...
    st.markdown(
        "**API:** [Open Docs]"
        "(https://sap-sales-agent-1.onrender.com/docs)"
    )
